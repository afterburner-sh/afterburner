...
/// dependencies, the stored digest must match exactly. Range-type dependencies
/// (`^0.1.0`, `>=1.0.0`) are version-checked by the caller (resolve_dep_closure
/// or pack_dir) before the linked source is composed; here we only verify
/// presence.
fn validate_pins(pins: &BTreeMap<String, String>, by_coord: &BTreeMap<&str, &Afb>) -> Result<()> {
    for (coord, value) in pins {
        let dep = by_coord
            .get(coord.as_str())
        ...
        let req = parse_dep_req(value).map_err(|e| AfbError::Dependency {
            coord: coord.clone(),
            detail: format!("invalid dependency value {value:?}: {e}"),
        })?;
        match req {
            DepReq::Pin(ref pin) => {
