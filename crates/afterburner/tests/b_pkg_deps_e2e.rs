...
fn new_ts_scaffold_then_package_is_js_only() {
    ...
    assert!(out.status.success(), "new --ts failed: {}", String::from_utf8_lossy(&out.stderr));
    assert!(dir.join("source/main.ts").exists(), "TS entry scaffolded");
    assert!(dir.join("tsconfig.json").exists(), "tsconfig scaffolded");
    assert!(!dir.join("source/main.js").exists(), "no JS entry for a TS package");

    // Pack it.
    let out = run(&["package", dir.to_str().unwrap(), "--out", dir.join("p.afb").to_str().unwrap()], None);
...
fn new_js_scaffold_packs_unchanged() {
    ...
    let out = run(
        &["init", dir.to_str().unwrap(), "--name", "thing", "--namespace", "acme"],
        None,
    );
...
fn declared_deps_are_not_packed_into_afb() {
    ...
    let out = run(
        &["init", dir.to_str().unwrap(), "--name", "app", "--namespace", "acme"],
        None,
    );
    ...
    std::fs::write(dir.join("node_modules/leftpad/index.js"), "module.exports=1;").unwrap();
    ...
    let afb = afterburner_afb::Afb::from_bytes(&bytes).unwrap();
    assert_eq!(afb.manifest.npm.get("leftpad").map(String::as_str), Some("^1.0.0"));
}
...
fn native_artifact_under_source_is_rejected_by_package() {
    ...
    let out = run(
        &["init", dir.to_str().unwrap(), "--name", "n", "--namespace", "acme"],
        None,
    );
    assert!(out.status.success(), "{}", String::from_utf8_lossy(&out.stderr));
    ...
    let out = run(&["package", dir.to_str().unwrap(), "--out", dir.join("p.afb").to_str().unwrap()], None);
    assert!(!out.status.success(), "package must reject a native artifact");
    let msg = String::from_utf8_lossy(&out.stderr);
    assert!(msg.to_lowercase().contains("native"), "error must name the native rejection: {msg}");
}
...
fn packing_is_fast() {
    ...
    assert!(run(&["init", dir.to_str().unwrap(), "--name", "f", "--namespace", "acme"], None)
        .status
        .success());
