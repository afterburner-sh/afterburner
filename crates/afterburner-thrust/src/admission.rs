...
impl TokenBucketAdmission {
    ...
    /// rejected by rounding. `governance` is applied to the sweep
    /// thread; `Err` when a non-default governance fails to apply
    /// (never silently - see `afterburner_core::governance`).
    pub fn new(
        tokens_per_sec: u64,
        burst_tokens: u64,
        governance: ThreadGovernance,
    ) -> afterburner_core::Result<Self> {
    ...
    pub fn new_with_intervals(
        ...
        let name = governance.thread_name("afterburner-admission-sweep", "");
        let sweep_thread = {
            let buckets = buckets.clone();
            let shutdown = sweep_shutdown.clone();
            let idle_threshold_ns = idle_threshold.as_nanos() as u64;
            spawn_governed(name, governance, move || {
...
mod tests {
    ...
    fn allows_up_to_burst_then_rejects() {
        ...
        let adm = TokenBucketAdmission::new(100, 5, ThreadGovernance::default()).unwrap();
        let t = tid(1);
        for i in 0..5 {
            adm.try_acquire(t)
    ...
    fn distinct_tenants_are_isolated() {
        ...
        let adm = TokenBucketAdmission::new(10, 1, ThreadGovernance::default()).unwrap();
        let a = tid(1);
        let b = tid(2);
    ...
    fn reports_sensible_retry_after() {
        ...
        let adm = TokenBucketAdmission::new(100, 1, ThreadGovernance::default()).unwrap();
        let t = tid(3);
    ...
    fn refill_allows_retry_after_waiting() {
        ...
        let adm = TokenBucketAdmission::new(500, 1, ThreadGovernance::default()).unwrap();
        let t = tid(4);
        adm.try_acquire(t).unwrap();
    ...
    fn zero_rate_does_not_panic() {
        ...
        let adm = TokenBucketAdmission::new(0, 0, ThreadGovernance::default()).unwrap();
        let t = tid(5);
    ...
    #[test]
    fn sweep_thread_uses_the_default_name_with_default_governance() {
        let (tx, rx) = kovan_channel::bounded::<String>(1);
        // Piggyback on `spawn_governed` directly to observe the exact
        // name `TokenBucketAdmission::new` would give its sweep thread
        // with no governance configured - proves the default posture is
        // byte-identical to the pre-governance name.
        let name = ThreadGovernance::default().thread_name("afterburner-admission-sweep", "");
        let handle = spawn_governed(name, ThreadGovernance::default(), move || {
            tx.send(std::thread::current().name().unwrap_or("").to_string());
        })
        .unwrap();
        handle.join().unwrap();
        assert_eq!(rx.recv(), Some("afterburner-admission-sweep".to_string()));
    }

    #[test]
    fn sweep_thread_honors_a_name_prefix() {
        let adm = TokenBucketAdmission::new_with_intervals(
            1_000,
            5,
            Duration::from_secs(60),
            Duration::from_secs(60),
            ThreadGovernance {
                name_prefix: Some("myapp-admission".to_string()),
                ..Default::default()
            },
        )
        .unwrap();
        // Constructing successfully with a custom prefix is the
        // observable proof here (the thread is already running by the
        // time `new_with_intervals` returns - `spawn_governed`
        // synchronously confirms governance before handing back).
        assert_eq!(adm.bucket_count(), 0);
    }
}
