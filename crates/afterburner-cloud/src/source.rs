...
mod tests {
    ...
    fn lists_candidates_with_deps_and_skips_yanked_dep_fetch() {
        ...
        let pkg = server.mock(|when, then| {
            when.method(GET).path("/api/v1/packages/burn/a");
            then.status(200).json_body(json!({
                "namespace": "burn", "name": "a",
                "versions": [
        ...
        let v1 = server.mock(|when, then| {
            when.method(GET).path("/api/v1/packages/burn/a/1.0.0");
            then.status(200).json_body(json!({
                "namespace": "burn", "name": "a", "version": "1.0.0", "digest": "aa",
                "dependencies": {"burn/b": "sha256:cc"}
            }));
        });

        let client = RegistryClient::new(server.base_url(), None);
        let src = RegistrySource::new(&client);
        let cands = src.candidates("burn/a").unwrap();
    ...
    fn resolves_a_small_graph_end_to_end() {
        ...
        server.mock(|when, then| {
            ...
            then.status(200).json_body(json!({
                "namespace": "burn", "name": "a", "version": "1.0.0", "digest": "aa",
                "dependencies": {"burn/b": "sha256:bb"}
            }));
        });
        server.mock(|when, then| {
            when.method(GET).path("/api/v1/packages/burn/b");
            then.status(200).json_body(json!({
                "namespace": "burn", "name": "b",
                "versions": [{"version": "1.0.0", "digest": "bb", "yanked": false}]
            }));
        });
        server.mock(|when, then| {
            when.method(GET).path("/api/v1/packages/burn/b/1.0.0");
            then.status(200).json_body(json!({
                "namespace": "burn", "name": "b", "version": "1.0.0", "digest": "bb",
                "dependencies": {}
            }));
        });
        ...
        let src = RegistrySource::new(&client);
        let res = resolve(
            &[("burn/a".to_string(), Req::from_cli_version(None).unwrap())],
            &src,
            &rt(),
        )
        .unwrap();
        ...
        assert_eq!(res.selected["burn/a"].version.to_string(), "1.0.0");
        assert_eq!(res.selected["burn/b"].version.to_string(), "1.0.0");
        // b is a dependency of a, so it must load first.
        let pos = |c: &str| res.order.iter().position(|x| x == c).unwrap();
        assert!(pos("burn/b") < pos("burn/a"));
    }
    ...
    fn missing_package_surfaces_not_found() {
        let server = MockServer::start();
        server.mock(|when, then| {
            when.method(GET).path("/api/v1/packages/burn/ghost");
            then.status(404).json_body(json!({"error": "not found"}));
        });
        let client = RegistryClient::new(server.base_url(), None);
        let src = RegistrySource::new(&client);
        assert!(matches!(
            src.candidates("burn/ghost"),
