...
mod tests {
    ...
    // ---- emscripten_get_now: deterministic clock ----------------------------

    /// `emscripten_get_now` returns the fixed virtual clock constant.
    #[test]
    fn get_now_returns_virtual_constant() {
        let vm = EmbedderVm::new().unwrap();
        // WAT: import env.emscripten_get_now, call it, reinterpret bits as i64.
        let module = vm
            .compile(
                &w(r#"
                  (module
        ...
        // Reinterpret the fixed constant back to f64 for assertion.
        let got = f64::from_bits(out.result as u64);
        assert_eq!(got, VIRTUAL_NOW_MS, "get_now must return VIRTUAL_NOW_MS");
    }

    /// Two runs return the same value (determinism check).
    #[test]
    ...
    // ---- emscripten_resize_heap: memory growth ------------------------------

    /// `emscripten_resize_heap` called with a size the module already has
    /// returns 1 (success without actually growing).
    #[test]
    fn resize_heap_already_large_enough() {
        let vm = EmbedderVm::new().unwrap();
        // One page = 65536 bytes; request exactly that - should succeed (already there).
        let module = vm
            .compile(
                &w(r#"
                  (module
        ...
        assert_eq!(
            out.result, 1,
            "resize to current size must return 1 (success)"
        );
    }

    /// `emscripten_resize_heap` with a larger size causes the memory to grow
    /// and returns 1.
    #[test]
    fn resize_heap_grows_memory() {
        let vm = EmbedderVm::new().unwrap();
        // Start with 1 page (65536 bytes); request 2 pages (131072 bytes).
        let module = vm
            .compile(
                &w(r#"
                  (module
            ...
            .unwrap();
        let out = vm.run(&module, "run", None).unwrap();
        assert_eq!(out.result, 1, "resize to 2 pages must return 1 (success)");
    }

    // ---- emscripten_memcpy_js: in-memory copy -------------------------------

    /// `emscripten_memcpy_js` copies bytes within wasm linear memory correctly.
    /// Write a known pattern at offset 0, copy to offset 64, read back via
    /// a wasm function that packs 4 bytes into an i32.
    #[test]
    fn memcpy_js_copies_bytes() {
        let vm = EmbedderVm::new().unwrap();
        // Layout:
        //   offset 0: "ABCD" (0x41 0x42 0x43 0x44)
        //   copy 4 bytes from 0 -> 64
        //   load i32 from offset 64 (little-endian: 0x44434241)
        let module = vm
            .compile(
                &w(r#"
                  (module
    ...
    // ---- combined: multiple env imports in one module ----------------------

    /// A module that imports abort + get_now + resize_heap all at once can
    /// be compiled and run; it calls get_now (the safe path) and returns the
    /// virtual constant without triggering abort or resize.
    #[test]
